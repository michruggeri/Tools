...
def build_excitation(state: list, hole: int, particle: int) -> str:
    """
...
def main() -> None:
    ...
    print(
        f"Generating determinants putting {number_of_electrons} electrons"
    ...
    with open("determinants.dat", "w") as output_file:
        while True:
            try:
                ...
                if (
                    hole >= number_of_states
                    or particle >= number_of_states
                    or hole < 0
                    or particle < 0
                ):
                    print(f"All input must be between 0 and" f" {number_of_states-1}")
                    continue
                ...
                break
    print("Output written to determinants.dat")
